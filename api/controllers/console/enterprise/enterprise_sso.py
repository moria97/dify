...
class EnterpriseSSOSamlAcs(Resource):
    ...
    def post(self):
        ...
        try:
            access_token, refresh_token = EnterpriseSSOService.post_sso_saml_acs(saml_response)
            return redirect(f'{current_app.config.get("CONSOLE_WEB_URL")}/signin?access_token={access_token}&refresh_token={refresh_token}')
...
class EnterpriseSSOOidcCallback(Resource):
    ...
    def get(self):
        ...
        try:
            access_token, refresh_token = EnterpriseSSOService.get_sso_oidc_callback(args)
            return redirect(f'{current_app.config.get("CONSOLE_WEB_URL")}/signin?access_token={access_token}&refresh_token={refresh_token}')
        except Exception as e:
            return redirect(f'{current_app.config.get("CONSOLE_WEB_URL")}/signin?message={str(e)}')
