...
from services.account_service import AccountService, TenantService, _generate_refresh_token
...
class EnterpriseSSOService:
    ...
    def get_sso_oidc_callback(cls, args: dict):
        ...
        response = EnterpriseRequest.send_request('GET', '/sso/oidc/callback', params={'code': code_from_query, 'state': state_from_query})
        if 'email' not in response or response['email'] is None:
            logger.exception(response)
